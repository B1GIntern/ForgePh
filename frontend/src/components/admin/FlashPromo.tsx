...
const FlashPromo: React.FC<FlashPromoProps> = ({ isAdmin = false, userId, onJoinPromo }) => {
  ...
  const fetchFlashPromos = async () => {
    try {
      setIsLoading(true);
      console.log('Fetching flash promos from:', '/api/flash-promos');
  ...
  const toggleFlashPromoStatus = async (id: string, currentStatus: boolean) => {
    try {
      ...
      setFlashPromos(flashPromos.map(promo => 
        promo._id === id ? {...response.data, status: !currentStatus ? 'ACTIVE' : 'INACTIVE'} : promo
      ));
  ...
  // Join a flash promo (for consumers)
  const handleJoinPromo = async (promoId: string) => {
    if (!userId) {
      toast({
        title: 'Error',
        description: 'You must be logged in to join a promo',
        variant: 'destructive'
      });
      return;
    }
    ...
    try {
      if (onJoinPromo) {
        ...
        // Refresh the list to show updated participation
        fetchFlashPromos();
      }
    } catch (error) {
