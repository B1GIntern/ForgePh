...
const Header: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const socket = io(SOCKET_URL || "https://forgeph-2.onrender.com");
  ...
  return (
    <>
      ...
      {isMobileMenuOpen && (
        <>
          <div 
            className="fixed inset-0 bg-[#0A0D14] z-[100] md:hidden" 
            onClick={toggleMobileMenu}
            style={{backgroundColor: '#0A0D14'}}
          />
          <div className="fixed inset-0 bg-[#0A0D14] z-[99] md:hidden"></div>
        </>
      )}
