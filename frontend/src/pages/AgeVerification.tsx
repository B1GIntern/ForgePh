...
const AgeVerification: React.FC = () => {
  ...
  // Check for redirect path from either router state or sessionStorage
  useEffect(() => {
    const savedRedirectPath = sessionStorage.getItem("redirectAfterVerification");
  ...
  const handleVerification = () => {
    ...
    if (age >= 18) {
      // Set age verification in both localStorage and sessionStorage
      localStorage.setItem("ageVerified", "true");
      sessionStorage.setItem("ageVerified", "true");
      
      // Navigate to the appropriate page based on various redirect sources
      // Priority: 1. sessionStorage redirect, 2. location state, 3. default to home
    ...
    <section className="min-h-screen flex items-center justify-center relative overflow-hidden pt-24 pb-16">
      ...
      <div className="container mx-auto px-6 flex items-center justify-center">
        <div className="flex-1 text-center z-10 max-w-md">
          ...
          {/* Show message if user was redirected */}
          {(location.state?.from || redirectPath) && (
            <div className="mb-6 text-amber-400 text-sm bg-amber-900/20 p-3 rounded-lg">
              You need to verify your age to access the requested page.
            </div>
          )}
