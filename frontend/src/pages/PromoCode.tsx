...
const PromoCodes: React.FC = () => {
  ...
  const fetchRetailers = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/promo-codes/retailers`, {
        headers: {
  ...
  const refreshRedemptionCount = async () => {
    try {
      ...
      const response = await fetch(
        `https://forgeph-2.onrender.com/api/promo-codes/check-redemptions/${userId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/promo-codes/redeem`, {
        method: "POST",
        headers: {
                ...
                <TabsContent value="flash" className="space-y-6 mt-6">
                  <FlashPromo isAdmin={false} userId={user?.id} onJoinPromo={async (promoId) => {
                    try {
                      ...
                      const response = await fetch(`${API_BASE_URL}/flash-promos/${promoId}/join`, {
                        method: "POST",
                        headers: {
                          "Content-Type": "application/json",
                          Authorization: `Bearer ${token}`
