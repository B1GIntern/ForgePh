...
// Get all redeemed promo codes
const getRedeemedPromoCodes = async (req, res) => {
  try {
    // Find promo codes that have been redeemed
    const redeemedCodes = await PromoCode.find({ 
      promoCodeRedeemed: true,
      "redeemedBy.consumerId": { $exists: true }
    });
    
    console.log(`Found ${redeemedCodes.length} redeemed promo codes`);
    
    return res.status(200).json({
      success: true,
      count: redeemedCodes.length,
      data: redeemedCodes
    });
  } catch (error) {
    ...
    return res.status(500).json({
      success: false,
      message: error.message || "Failed to fetch redeemed promo codes",
      data: []
    });
  }
};
