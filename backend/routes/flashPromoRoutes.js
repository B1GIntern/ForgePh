...
router.patch("/:id/status", async (req, res) => {
  try {
    ...
    res.json({ success: true, ...updatedFlashPromo.toObject() });
  } catch (err) {
...
// POST endpoint for updating flash promo status (to avoid CORS issues)
...
// Update a flash promo (PUT endpoint for the entire object)
  ...
  try {
    ...
    allowedUpdates.forEach(field => {
      if (req.body[field] !== undefined) {
        flashPromo[field] = req.body[field];
      }
    });
    ...
    console.error("Error updating flash promo:", err);
    res.status(500).json({ message: err.message });
  }
});
